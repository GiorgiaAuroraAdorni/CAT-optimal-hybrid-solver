...
class GameEnvironment(gym.Env):
    def __init__(self, V,voidMat, max_id, instructions, patterns, num_colors,map_value, n):
        ...
        self.observation_space = spaces.Box(low=-2, high=num_colors, shape=(n, 2*n), dtype=np.int)
    ...
    def reset(self):
        ...
        next_state = np.concatenate((self.V, self.currentMat), axis=-1)
        return next_state   
    ...
    def step(self, action):
        ...
        next_state = np.concatenate((self.V, next_state), axis=-1)
