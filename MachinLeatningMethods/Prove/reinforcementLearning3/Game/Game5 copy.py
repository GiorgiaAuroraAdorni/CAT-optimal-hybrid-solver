...
class GameEnvironment(gym.Env):
    def __init__(self, V,voidMat, max_id, instructions, patterns, num_colors,map_value, n):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(ZOOM_COEFF * n, 2 * ZOOM_COEFF * n, 3), dtype=np.int64)
    ...
    def step(self, action):
        ...
        if self.V[node_i][node_j] == -1:
            state = self.get_state()
            return state, -3, self.is_done(),{'current_id': self.current_id}
